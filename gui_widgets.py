...
class ModelManager:
    ...
    def checkpoint_path(self, model_name: str, dataset: str) -> Path:
        if model_name == "LSTM":
            return ROOT / "results" / f"best_lstm_{dataset}_best_acc.pth"
    ...
    def predict(
        self, model_name: str, dataset: str, sequence: np.ndarray
    ) -> PredictionResult:
        return self._predict_one(model_name, dataset, sequence)

    def _predict_one(self, model_name: str, dataset: str, sequence: np.ndarray,
        ...
        if x.ndim != 2:
            raise ValueError(f"LSTM 输入应为二维时序特征，实际形状：{x.shape}")
        if x.shape[1] != model.input_dim:
            raise ValueError(
                f"LSTM 特征维度不匹配：模型需要 {model.input_dim}，实际为 {x.shape[1]}"
            )
        if mean is not None and std is not None:
            x = (x - mean) / (std + 1e-8)
        x_tensor = torch.as_tensor(x[None, :, :], dtype=torch.float32, device=self.device)
        lengths = torch.as_tensor([len(x)], dtype=torch.long, device=self.device)
        with torch.no_grad():
            if return_embedding:
                logits, embedding = model(
                    x_tensor, lengths, return_embedding=True
                )
    ...
    def predict_with_embedding(self, model_name: str, dataset: str,
                                sequence: np.ndarray) -> PredictionResult:
        ...
        return self._predict_one(
            model_name, dataset, sequence, return_embedding=True
        )
    ...
    def compute_standard_embeddings(self, model_name: str, dataset: str,
        ...
        """
        from audio_feature import extract_mfcc_sequence
        ...
        for wav_path in sorted(standard_audio_dir.glob("*.wav")):
            ...
            if len(label) == 1 and label.isalpha():
                try:
                    ...
                    sequence = extract_mfcc_sequence(audio, sr)
                    result = self.predict_with_embedding(
                        model_name, dataset, sequence
                    )
